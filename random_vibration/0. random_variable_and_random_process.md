...
$$ P(\alpha < X < \beta) = \frac{1}{\sqrt{\pi}} \int_{\frac{\alpha - \mu}{\sigma\sqrt{2}}}^{\frac{\beta - \mu}{\sigma\sqrt{2}}} e^{-t^2} dx $$
    ...
    引入拉普拉斯函数$\Phi$
$$ \Phi(x) = \frac{2}{\sqrt{\pi}} \int_{0}^{x} e^{-t^2} dt $$

    于是
$$ P(\alpha < X < \beta) = \frac{1}{2}\left [\Phi \left [\frac{\beta -\mu}{\sigma \sqrt{2}}\right ] - \Phi \left [\frac{\alpha -\mu}{\sigma \sqrt{2}} \right ] \right ] $$

    拉普拉斯函数$\Phi$是$x$的奇函数，且$\Phi(0)=0, \Phi(\infty)=1$，拉普拉斯函数一般预先制成表用于查询。
